...
export const getAllCategories = unstable_cache(
  async (): Promise<CategoryRow[]> => {
    ...
    return [
      ...
      { id: "cat-3", slug: "blankets_mattresses", name_ar: "فرشة وأغطية", default_unit: "piece", sort_order: 3, created_at: new Date().toISOString() },
...
export const getAllReliefHubs = unstable_cache(
  async (): Promise<ReliefHubRow[]> => {
    try {
      const supabase = await createClient();
  ...
  ["admin-stats", "relief-hubs"],
  { revalidate: 300, tags: ["admin-stats", "master-data"] },
);

export const getAllCollectionPoints = unstable_cache(
  async (): Promise<CollectionPointRow[]> => {
    try {
      const supabase = await createClient();
  ...
  ["admin-stats", "collection-points"],
  { revalidate: 300, tags: ["admin-stats", "master-data"] },
);

export const getTopCriticalNeeds = unstable_cache(
  async (limit = 6): Promise<unknown[]> => {
    try {
      const supabase = await createClient();
      const { data } = await supabase
        .from("needs")
    ...
    return [
      {
        id: "need-1",
        campaign_id: "demo-campaign",
        title: "فرشة وأغطية شتوية عاجلة للأسر المنكوبة",
        wilaya: "جيجل",
        ...
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString(),
        categories: { slug: "blankets_mattresses", name_ar: "فرشة وأغطية" },
      },
      {
        id: "need-2",
        campaign_id: "demo-campaign",
  ...
  ["admin-stats", "top-critical-needs"],
  { revalidate: 60, tags: ["admin-stats"] },
);

export const getPendingVerificationCounts = unstable_cache(
  async () => {
    try {
      const supabase = await createClient();
  ...
  ["admin-stats", "pending-verification"],
  { revalidate: 60, tags: ["admin-stats"] },
);

export const getNeedsByPriority = unstable_cache(
  async () => {
    try {
      const supabase = await createClient();
      const { data } = await supabase.rpc("count_needs_by_priority");

      const counts: Record<string, number> = { critical: 0, high: 0, medium: 0, low: 0 };
      for (const row of data ?? []) {
        counts[row.priority] = Number(row.count) || 0;
      }
      return counts;
    } catch {
  ...
  ["admin-stats", "needs-by-priority"],
  { revalidate: 60, tags: ["admin-stats"] },
);

export const getActivityTrend = unstable_cache(
  async (days = 14) => {
    try {
      const supabase = await createClient();
...
export const getWeekOverWeekDelta = unstable_cache(
  async () => {
    try {
      const supabase = await createClient();
      const now = new Date();
      ...
      return { donationsDeltaPct: 0, needsDeltaPct: 0 };
    }
  },
  ["admin-stats", "week-over-week"],
  { revalidate: 30, tags: ["admin-stats"] },
);
